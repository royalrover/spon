...
Tmod.prototype = {
    ...
    getConfig: function () {
        ...
        // 更新模板项目的依赖版本信息
        json.dependencies.tmodjs = json.version;
    ...
    compile: function (file) {
        ...
        if (file) {
            ...
            walk = function (dir) {
                ...
                dirList.forEach(function (item) {
                    ...
                    if (fs.statSync(path.join(dir, item)).isDirectory()) {
                        ...
                        if (!error && that.options.type == 'commonjs') {
                
                            // 目标路径
                            var fileName = path.join(dir, item);
                            var target = fileName
                            .replace(path.extname(fileName), '.js')
                            .replace(that.base, that.output);    
            ...
            // 删除生成的配置文件
            fs.unlinkSync(path.join(this.base,'package.json'));
    ...
    // 编译运行时
    _buildRuntime: function (file,templates, metadata, callback) {
        ...
        try {
            ...
            fs.unlinkSync(file);
            fs.renameSync(this.runtime,file);
        } catch (e) {
            error = e;
        }
...
*/
    ...
    _combo: function (file) {
        ...
        this._buildRuntime(file,combo, metadata, function (error, data) {
