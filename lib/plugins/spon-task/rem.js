 ...
 */
module.exports = function(remConfig){
  ...
  // 根据配置，设置rem转换
  var updateRemWithConfig = function(){
  ...
  var updateRemWithAnnotationAndConfig = function(){
    var defaults = {
      rootValue: 75,
      unitPrecision: 4,
      /*whiteList: ['font','font-size','line-height', 'letter-spacing','text-indent',
        'word-spacing','width','height','max-height','max-width','min-height','min-width',
        'left','top','right','bottom','margin','margin-left',
        'margin-top','margin-right','margin-bottom','padding','padding-left',
        'padding-top','padding-right','padding-bottom','background','background-position',
    ...
    var opts;
    if(remConfig){
      opts = _.extend({}, defaults,remConfig);
    }else{
      opts = _.extend({}, defaults);
    }

    // 获取参数传递的黑名单列表
    var blacklist = Object.prototype.toString.call(opts.blackList) == '[object Array]' ? opts.blackList : null;
    ...
    px2rem = postcss.plugin('postcss-px2rem', function(){
      ...
      return function (css) {
        ...
        css.walkDecls(function (decl, i) {
          // 黑名单过滤
          if(blacklist && blacklist.length && blacklist.indexOf(decl.prop) != -1) return;

          if(decl.raws.value && decl.raws.value.raw.indexOf('/*@norem*/') !== -1) return;
  ...
  return updateRemWithAnnotationAndConfig;
