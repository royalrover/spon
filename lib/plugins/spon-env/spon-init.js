...
module.exports = {
  init: function(){
    ...
    'gulp-less','gulp-minify-css','gulp-notify','gulp-rename','gulp-webpack','gulp-htmlhint','gulp-jshint','jshint','less-loader',
    'npmlog','request','style-loader','lodash','webpack'];
    ret = shelljs.exec('npm root -g',{silent:true});
    if(ret.code !== 0){
      npmlog.error('spon:mobi: ','spon init encounter an error when execute "npm root -g", confirm you have installed "node,npm"!');
    }
  ...
  __install: function(){
    if(shelljs.exec('which node',{silent: true}).output.indexOf('/') == -1){
      npmlog.error('spon:mobi: ','检测到当前环境并未安装node，请您去nodejs社区寻找相关资源安装！');
      return 1;
    }
    // 提示cnpm安装文案
    if(shelljs.exec('which cnpm',{silent: true}).output.indexOf('/') == -1){
      npmlog.error('spon:mobi: ','检测到当前环境并未安装cnpm命令，为了您更便捷的安装使用npm的模块，十分建议您安装cnpm。\n您可以使用cnpm命令行工具代替默认的npm，安装方法：\nnpm install -g cnpm --registry=https://registry.npm.taobao.org');
      return 1;
    }

    npmlog.info('spon:mobi: ','检查并安装相关插件(gulp，yoman)');

    if(shelljs.exec('which gulp',{silent: true}).output.indexOf('/') == -1){
      npmlog.info('spon:mobi: ','安装gulp命令');
      if(shelljs.exec('cnpm --loglevel info i -g gulp').code !== 0){
        npmlog.error('spon:mobi: ','安装gulp命令出错，请手动安装！');
        return 1;
      }
    }
    if(shelljs.exec('which yo',{silent: true}).output.indexOf('/') == -1){
      npmlog.info('spon:mobi: ','安装yo命令');
      if(shelljs.exec('cnpm --loglevel info i -g yo').code !== 0){
        npmlog.error('spon:mobi: ','安装yo命令出错，请手动安装！');
        return 1;
      }
    }
