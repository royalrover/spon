  ...
  if(!this.isDev){
    ...
    this.prompt(prompts, function (props) {
      ...
      if(this.msg.replace(/\s+/g,'').length == 0){
        npmlog.error('spon:mobi: ','you should input commit message!');
        return 1;
      }
      if(!/^\d+\.\d+\.\d+/g.test(this.tag)){
        npmlog.error('spon:mobi: ','format of the tag is wrong, FORMAT: x.x.x');
        return 1;
      }
      ...
      npmlog.info('spon:pc: ','modify version info for spm!');

      if(shelljs.exec('git add .').code !== 0){
        npmlog.error('spon:pc: ','publish encounter an error when execute "git add ."!');
        return 1;
      }
      if(shelljs.exec('git status',{silent: true}).output.indexOf('nothing to commit, working directory clean') == -1){
        if(shelljs.exec('git commit -m " ' + this.msg + '"').code !== 0){
          npmlog.error('spon:pc: ','publish encounter an error when execuate "git commit"!');
          return 1;
        }
      }

      if(shelljs.exec('git push origin master').code !== 0){
        npmlog.error('spon:pc: ','publish encounter an error when execute "git push"!');
        return 1;
      }
      ...
      if(shelljs.exec('git tag '+ this.tag).code !== 0){
        npmlog.error('spon:pc: ','publish encounter an error when execute "git tag tagname"!');
        return 1;
      }
      if(shelljs.exec('git push  origin '+ this.tag).code !== 0){
        npmlog.error('spon:pc: ','publish encounter an error when execute "git push tag"!');
        return 1;
      }

      npmlog.info('spon:pc: ','publish to online env successfully, current tag: ' + this.tag);
      cb();
    }.bind(this));
  }else{
    // 测试
    ...
    this.prompt(prompts, function (props) {
      ...
      if(this.msg.replace(/\s+/g,'').length == 0){
        npmlog.error('spon:pc: ','you should input commit message!');
        return 1;
      }
      if(!/^\d+\.\d+\.\d+/g.test(this.tag)){
        npmlog.error('spon:pc: ','format of the tag is wrong, FORMAT: x.x.x');
        return 1;
      }
      if(shelljs.exec('git add .').code !== 0){
        npmlog.error('spon:pc: ','publish encounter an error when execute "git add ."!');
      ...
      npmlog.info('spon:pc: ','publish to dev env successfully, current tag: ' + this.tag);
      cb();
    }.bind(this));
  }
