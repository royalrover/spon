  ...
  if(!this.isDev){
    ...
    this.prompt(prompts, function (props) {
      ...
      if(!/^publish\/\d+\.\d+\.\d+/g.test(this.tag)){
        npmlog.error('spon:mobi: ','format of the tag is wrong, FORMAT: publish/x.x.x');
        return 1;
      }
      ...
      if(shelljs.exec('git status',{silent: true}).output.indexOf('nothing to commit, working directory clean') == -1){
        if(shelljs.exec('git commit -m " ' + this.msg + '"').code !== 0){
          npmlog.error('spon:mobi: ','mobi-page project publish encounter an error when execuate "git commit"!');
          return 1;
        }
      }

      if(shelljs.exec('git push origin master').code !== 0){
        npmlog.error('spon:mobi: ','mobi-page project publish encounter an error when execute "git push"!');
        return 1;
      }
      if(shelljs.exec('git tag '+ this.tag).code !== 0){
        npmlog.error('spon:mobi: ','mobi-component project publish encounter an error when execute "git tag tagname"!');
        return 1;
      }
      if(shelljs.exec('git push  origin '+ this.tag).code !== 0){
        npmlog.error('spon:mobi: ','mobi-component project publish encounter an error when execute "git push tag"!');
        return 1;
      }

      npmlog.info('spon:mobi: ','publish to online env successfully, current tag: ' + this.tag);
      cb();
    }.bind(this));
  }else{
    // 测试
    ...
    this.prompt(prompts, function (props) {
      ...
      if(!/^dev\/\d+\.\d+\.\d+/g.test(this.tag)){
        npmlog.error('spon:mobi: ','format of the tag is wrong, FORMAT: dev/x.x.x');
        return 1;
      }
      if(shelljs.exec('git status',{silent: true}).output.indexOf('nothing to commit, working directory clean') == -1) {
        if(shelljs.exec('git commit -m " ' + this.msg + '"').code !== 0){
      ...
      npmlog.info('spon:mobi: ','publish to test env successfully, current tag: ' + this.tag);
      cb();
    }.bind(this));
  }
