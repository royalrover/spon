      ...
      if (prompts.length) {
        ...
        this.prompt(prompts, function (props) {
          ...
          fs.writeFile('spon.json',JSON.stringify(spon,null,2),function(err){
            ...
            npmlog.info('spon: ','creat spon.json successfully');
            cb();
          });
        }.bind(this));
      }
...
sponGenerator.prototype.writing = function(){
  ...
  switch(self.type){
    ...
    case 'ReactNative':
      break;
    case 'midproxy':
      self.bulkDirectory('midproxy/api/mock/', 'api/mock/');
      self.bulkDirectory('midproxy/api/online/', 'api/online/');
      self.bulkDirectory('midproxy/pre/', 'pre/');
      self.bulkDirectory('midproxy/routes/api/', 'routes/api/');
      self.bulkDirectory('midproxy/routes/page/', 'routes/page/');
      self.bulkDirectory('midproxy/rules/', 'rules/');
