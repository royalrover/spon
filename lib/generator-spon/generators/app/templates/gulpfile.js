...
gulp.task('fetch', function(){
  ...
  // 加载模块及其依赖模块
  var recurseLoad = function(c,callback){
    ...
    if(deps[c]['requires'].length){
      ...
      async.map(deps[c]['requires'], function(d,cb){
        if(fs.existsSync(path.join(showjoyBase,d.split('/')[1] + '.js'))){
          return;
        }
      //  console.log(deps[c]['requires']);
        ...
        // 加载该模块
        request(base + deps[c]['alias'],function(error,res,body){
          var p;
          if(!error && res.statusCode == 200){
            log.info('spon:mobi:fetch get: ',base + deps[c]['alias']);
            p = path.join(showjoyBase,c.split('/')[1] + '.js');
            if(fs.existsSync(p)){
              fs.unlinkSync(p);
            }
    ...
    request(base + deps[c]['alias'],function(error,res,body){
      var p;
      if(!error && res.statusCode == 200){
        log.info('spon:mobi:fetch: ',base + deps[c]['alias']);
        p = path.join(showjoyBase,c.split('/')[1] + '.js');
        if(fs.existsSync(p)){
          fs.unlinkSync(p);
        }
