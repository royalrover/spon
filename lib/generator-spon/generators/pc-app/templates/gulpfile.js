      ...
      request(relDepUrl,function(error,res,body){
        ...
        if(!error && res.statusCode == 200){
          ...
          fs.writeFile(relDepPath, body,function(err){
            ...
            log.info('spon:mobi:fetch relative deps: ' + relDepUrl + ' successfully');
            cb();
          });
        }
      });
...
var webpack = require(path.join(baseNPMPath,'gulpp-webpack'));
...
gulp.task('fetch', function(asyncCb){
  ...
  try{
    ...
    dirs = dirs.concat(fs.readdirSync('src/components'));
  }catch(e){
  ...
  var loadFormDirs = function(dirs){
    ...
    dirs.forEach(function(item){
      ...
      if(fs.existsSync(p)){
        ...
        p = path.join('src/components',item,item + '.js');
        content = fs.readFileSync(p, 'utf-8');
      }
      ...
      // 由于采用“组建缓存”策略，针对缓存的组件，需要解析其依赖，并判断是否加载依赖
...
gulp.task('page-watch', function () {
  gulp.watch(['./src/**/*.less'],['parse-less']);
...
gulp.task('parse', function() {
  ...
  require('child_process').execSync('node ' + command + ' src/ --output src/ --no-watch --no-cache');
});
gulp.task('page-default', ['clean', 'fetch', 'lint', 'page-build', 'page-css', 'page-html']);
gulp.task('s',['serve', 'page-watch']);
gulp.task('parse-tmpl',['parse']);
