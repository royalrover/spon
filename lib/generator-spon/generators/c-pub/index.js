  ...
  if (prompts.length) {
    ...
    this.prompt(prompts, function (props) {
      ...
      if(shelljs.exec('git commit -m " ' + this.msg + '"').code !== 0){
        error('ERR: mobi-component project publish encounter an error when execuate "git commit"!');
        return 1;
      }

      if(shelljs.exec('git push origin master').code !== 0){
        error('ERR: mobi-component project publish encounter an error when execute "git push"!');
        return 1;
      }
      ...
      try{
        ...
        error('ERR: mobi-component project publish encounter an error when execute "git tag"!');
      }
      ...
      log('INFO: publish successfully, current tag: publish/0.0.' + pTag);
      cb();
    }.bind(this));
  }
};
