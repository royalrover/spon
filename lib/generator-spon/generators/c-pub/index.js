...
var error = function(msg){
  npmlog.error('spon:mobi: ',msg);
};

var log = function(msg){
  npmlog.info('spon:mobi: ',msg);
};
  ...
  if (prompts.length) {
    ...
    this.prompt(prompts, function (props) {
      ...
      if(shelljs.exec('git commit -m " ' + this.msg + '"').code !== 0){
        npmlog.error('spon:mobi: ','mobi-component project publish encounter an error when execuate "git commit"!');
        return 1;
      }

      if(shelljs.exec('git push origin master').code !== 0){
        npmlog.error('spon:mobi: ','mobi-component project publish encounter an error when execute "git push"!');
        return 1;
      }
      ...
      try{
        ...
        npmlog.error('spon:mobi: ','mobi-component project publish encounter an error when execute "git tag"!');
      }
      ...
      npmlog.info('spon:mobi: ','publish successfully, current tag: publish/0.0.' + pTag);
      cb();
    }.bind(this));
  }
};
